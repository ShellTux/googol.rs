...
fn benchmark_add_search(c: &mut Criterion) {
    ...
    c.bench_function("add_search 100000 times", |b| {
        b.iter(|| {
...
fn benchmark_count(c: &mut Criterion) {
    ...
    let test_words: Vec<&String> = words.choose_multiple(&mut rng(), 100).collect();

    c.bench_function("count 100 words", |b| {
        b.iter(|| {
...
fn benchmark_top_n(c: &mut Criterion) {
    ...
    c.bench_function("top_n 10 large dataset", |b| {
        b.iter(|| {
